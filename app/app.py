...
def artists(time_range):
    ...
    user_info = sp.current_user() #pulls user_info
    user_top_tracks = get_top_5_tracks(access_token, time_range=time_range)
    recommended_tracks = recommend.get_nearest_songs(user_top_tracks) #CHANGE TO ARTIST PARAMETER
    artist_urls = []
    seen_urls = set()

    for rec in recommended_tracks:
        curr_url = search_artist(artist = rec[1])
        if curr_url != "No results found." and curr_url not in seen_urls:
            artist_urls.append(curr_url)
            seen_urls.add(curr_url)
    ...
    #redirects to a html file with your username being displayed
    return render_template("artists.html", user_name = user_info["display_name"],
                           artist_url_1 = artist_urls[0], artist_url_2 = artist_urls[1],
                           artist_url_3 = artist_urls[2], artist_url_4 = artist_urls[3],
                           artist_url_5 = artist_urls[4])
...
def search_artist(artist):
    ...
    results = client_auth.search(q=query, type='artist', limit=1)
